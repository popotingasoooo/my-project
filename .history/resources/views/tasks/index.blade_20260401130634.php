        ...
        <div class="surface-card p-1">
            <div class="flex">
                <a href="{{ route('tasks.index', ['view' => 'board']) }}"
                   class="flex-1 text-center px-4 py-3 text-sm font-medium rounded-2xl transition {{ request('view', 'board') === 'board' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:text-slate-900' }}">
                    Board
                </a>
                <a href="{{ route('tasks.index', ['view' => 'history']) }}"
                   class="flex-1 text-center px-4 py-3 text-sm font-medium rounded-2xl transition {{ request('view', 'board') === 'history' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:text-slate-900' }}">
                    History
                </a>
            </div>
        </div>

        @if(request('view', 'board') === 'board')
