...
class UserController extends Controller
{
    ...
    public function update(Request $request, User $user)
    {
        ...
        if (auth()->id() === $user->id) {
            $freshUser = User::find($user->id); // Get the fresh user data from the database
            auth()->setUser($freshUser); // Update the authenticated user with the fresh data to reflect role changes immediately
            app()[\Spatie\Permission\PermissionRegistrar::class]->forgetCachedPermissions(); // Clear cached permissions again to ensure changes take effect
        }
    }
